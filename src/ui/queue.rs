...
pub(crate) fn contents(app: &mut App, ui: &mut egui::Ui, compact: bool) {
    ...
    if queue_is_empty(app) {
        widgets::empty_state(
            ...
            &palette,
            Icon::ListVideo,
            &tr!(app.locale, "Nothing queued"),
            &tr!(app.locale, "Queued songs appear here."),
        );
        return;
    }
    ...
    if items.is_empty() {
        ...
        if loaded_once {
            widgets::empty_state(
                ...
                &palette,
                Icon::Clock,
                &tr!(app.locale, "No recent plays"),
                &tr!(app.locale, "Played songs appear here."),
            );
        } else {
            widgets::loading_row(ui, &palette);
        }
